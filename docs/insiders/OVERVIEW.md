...
| **Insiders is unstable** | Rollback is one-click (see [Rollback Guide](./ROLLBACK.md)); revert is faster than a fix |
...
- ✅ Rollback workflow added with hardened execution
...
1. **For engineers:** See [Workflow Guide](./WORKFLOW.md) for PR workflow
2. **For QA:** See [Testing Guide](./TESTING.md) for testing procedures
3. **For rollback:** See [Rollback Guide](./ROLLBACK.md) for emergency procedures
4. **For monitoring:** See [Monitoring](./MONITORING.md) for deployment traceability

## Questions?

- How do I deploy to insiders? → [Workflow Guide](./WORKFLOW.md)
- How do I verify what's live? → [Testing Guide](./TESTING.md)
- How do I revert a bad deployment? → [Rollback Guide](./ROLLBACK.md)
- Where do I see deployment info? → [Monitoring](./MONITORING.md)
