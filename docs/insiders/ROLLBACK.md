...
- ✓ Verify commit ancestry (is it on the insiders branch?)
- ✓ Create a revert commit (handles merge commits automatically)
...
- **Strict commit verification:** Confirms commit exists and is on insiders branch ancestry
- **Merge-commit handling:** Automatically detects merge commits and uses proper `-m 1` flag
- **Conflict detection:** Aborts gracefully if revert encounters conflicts
- **Full history access:** Uses complete git history for safe ancestry checks
...
**For merge commits:** If the commit has multiple parents, specify which parent to revert against:
```bash
git log --oneline | head -10  # Find the merge commit
git revert -m 1 abc1234 --no-edit  # -m 1 specifies the mainline parent
```
...
# If there are conflicts (both modified):
...
- Questions about merge commits? See "Handling Revert Conflicts" section
