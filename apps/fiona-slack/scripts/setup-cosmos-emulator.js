...
// Build CosmosClient using shared utility, falling back to the well-known
// emulator endpoint + key when no credentials are configured.
// The fixed key is used by every default Cosmos DB Emulator install.
...
const EMULATOR_KEY = 'C2y6yDjf5/R+ob0N8A7Cgv30VRDJIWEHLM+4QDU5DE2nQ9nDuVTqobD4b5n5MBLzPU1z+OhS8OyX8+tU9J1A==';

const config = getCosmosConfig({
  endpoint: process.env.COSMOS_ENDPOINT || EMULATOR_ENDPOINT,
...
if (!client) {
  console.error('Failed to create CosmosClient. Check your environment configuration.');
  process.exit(1);
}
