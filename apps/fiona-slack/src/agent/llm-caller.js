...
import { formatCitations } from './tools/format-citations.js';
...
async function callPerplexityChat(streamer, prompts) {
  ...
  // Citations are a Perplexity-specific top-level field on each chunk.
  // They are consistent across all chunks, so we capture on first occurrence.
  // We append formatted citations only after the full stream completes to
  // ensure the source list is always complete and deterministic (strict consistency).
  /** @type {string[]} */
  let citations = [];

  for await (const chunk of response) {
    // Capture citations from the first chunk that carries them.
    if (citations.length === 0 && Array.isArray(chunk.citations) && chunk.citations.length > 0) {
  ...
  // Append formatted citations after the entire stream has finished so that
  // the source list is always presented as a complete, numbered block.
  const citationText = formatCitations(citations);
  if (citationText) {
    await streamer.append({ markdown_text: citationText });
  }
}
    ...
    for (const call of toolCalls) {
      ...
      if (!args) {
        ...
        try {
          ...
          const searchCitations = Array.isArray(searchResponse.citations) ? searchResponse.citations : [];
          result = {
            output: searchResponse.choices[0].message.content,
