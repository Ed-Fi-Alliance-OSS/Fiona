...
async function getContainer(logger) {
  ...
  const config = getCosmosConfig();
  const conversationsContainer = process.env.COSMOS_CONVERSATIONS_CONTAINER || 'conversations';
  ...
  container = client.database(config.database).container(conversationsContainer);
  return container;
}
